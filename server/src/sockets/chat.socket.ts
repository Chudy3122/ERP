...
import { broadcastNewMessage } from './chatBroadcast';
  ...
  io.on('connection', (socket: AuthenticatedSocket) => {
    ...
    socket.on('chat:send_message', async (data: SendMessageData) => {
      try {
        ...
        // Deliver to every member via their personal room (reliable: always joined)
        await broadcastNewMessage(io, savedMessage.id);
